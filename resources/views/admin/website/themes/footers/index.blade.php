@extends('admin.layouts.app')

@push('css_custom')
<style>
    
tbody tr td img{
    width: 100px;
}
.button_cus_up{
    margin-top: 25px;
}
</style> 
@endpush
@push('css_lib')
    <!-- DataTables -->
    <link rel="stylesheet" href="{{ URL::asset('backend/bower_components/datatables.net-bs/css/dataTables.bootstrap.min.css') }}">
@endpush

@section('meta')
    <meta name="csrf-token" content="{{ csrf_token() }}" />

@endsection

@section('content')
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Footer
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Footer Management</a></li>
            <li class="active">Footer</li>
        </ol>
    </section>

    <!-- Main content -->
    <section class="content">
        <div class="row">
            <div class="col-xs-12">

                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">Footer</h3>
                        <div id="msg"></div>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        @if(isset($footer))
                        <form action="{{route('footers.update',['footer' => $footer->id])}}" method="post" enctype="multipart/form-data">
                            @csrf
                            {{method_field('PUT')}}
                            <div class="row">
                                <div class="col-md-4">
                                    <label for="developer"> Developer: </label>
                                    <input class="form-control" type="text" name="developer" value="{{$footer->developer}}" />
                                </div>
                                <div class="col-md-4">
                                    <label for="credit"> Credit :</label>
                                    <input class="form-control" type="text" name="credit" value="{{$footer->credit}}"  />
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <input type="submit" class="button_cus_up btn btn-block btn-success pull-left" value="Update">
                                    </div>
                                </div>
                            </div>
                        </form>

                        @else

                        <form action="{{route('footers.store')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-md-4">
                                    <label for="developer"> Developer: </label>
                                    <input class="form-control" type="text" name="developer" />
                                </div>
                                <div class="col-md-4">
                                    <label for="credit"> Credit :</label>
                                    <input class="form-control" type="text" name="credit" />
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <input type="submit" class="button_cus_up btn btn-block btn-success pull-left" value="Save">
                                    </div>
                                </div>
                            </div>
                        </form>

                        @endif
                    </div>

                    <div class="box-body">
                        {{-- <table id="example1" class="table table-bordered table-striped">
                            <thead>
                            <tr>
                                <th>SL#</th>
                                <th>Developer</th>
                                <th>Credit</th>
                                <th>Creator</th>
                                <th>Status</th>
                                <th>Action</th>
                            </tr>
                            </thead>
                            <tbody>
                            @if($footers)
                                @foreach($footers as $key => $footer)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $footer->developer }}</td>
                                    <td>{{ $footer->credit }}</td>
                                    <td>{{ $footer->user->name }}</td>
                                    <td>                                        
                                        @if($footer->status == 1) 
                                        <a href="{{route('footers.toogle',['id'=>$footer->id])}}" class="btn btn-xs btn-danger inactivate">Inactivate</a> 
                                        @elseif($footer->status == 0)  
                                        <a href="{{route('footers.toogle',['id'=>$footer->id])}}" class="btn btn-xs btn-success activate">Activate</a>
                                        @endif
                                    </td>
                                    <td>                                    
                                        <a href="{{ route('footers.destroy',$footer) }}" class="btn btn-xs btn-danger" onclick="if(confirm('Are you really want to delete this?')){
                                            event.preventDefault();
                                            document.getElementById('delete-form-{{ $footer->id }}').submit();
                                            }else{
                                            event.preventDefault();
                                            }"
                                        ><i class="fa fa-trash"></i></a>
                                        <form method="POST" id="delete-form-{{ $footer->id }}" action="{{ route('footers.destroy', $footer->id) }}" style="display:none">@csrf @method('DELETE')</form>
                                    </td>
                                </tr>
                                @endforeach
                            @endif
                            </tbody>
                        </table> --}}
                    </div>
                    <!-- /.box-body -->
                </div>
                <!-- /.box -->
            </div>
            <!-- /.col -->
        </div>
        <!-- /.row -->
    </section>
    <!-- /.content -->
    <!-- Modal -->
    <div id="logo_view" class="modal fade" role="dialog">
        <div class="modal-dialog">

            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Draft Title</h4>
                    <hr>
                    <h6 class="modal-category">Draft Category</h6>
                    <hr>
                </div>
                <div class="modal-body">
                    <p>Draft Content here...</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                </div>
            </div>

        </div>
    </div>
@endsection

@push('js_lib')
    <!-- DataTables -->
    <script src="{{ URL::asset('backend/bower_components/datatables.net/js/jquery.dataTables.min.js') }}"></script>
    <script src="{{ URL::asset('backend/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js') }}"></script>


@endpush

@push('js_custom')
    <script>
        $(function () {
            $('#example2').DataTable({
              'paging'      : true,
              'lengthChange': false,
              'searching'   : false,
              'ordering'    : true,
              'info'        : true,
              'autoWidth'   : false
            });

        });
    </script>
@endpush
